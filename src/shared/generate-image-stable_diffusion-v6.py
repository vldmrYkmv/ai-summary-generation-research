...
API_KEY = "Ijyha4LcB4N4MgztzlTOD37UShP9MpCFPmXUhn3qbvdE9gq2liaR7ywoCMOW"
...
if __name__ == '__main__':
  try:
    ...
    files_list = get_files_list(RESULTS_DIRECTORY_NAME_STUDY_2, False)
    print(f'Found {len(files_list)} directories in "{RESULTS_DIRECTORY_NAME_STUDY_2}" directory...')
    # for index, directory_name in enumerate(files_list):
    for index, directory_name in enumerate(files_list):
      print(f'[{index + 1}/{len(files_list)}] Starting sequence for "{directory_name}" book...')
      pages_list = get_files_list(f"{RESULTS_DIRECTORY_NAME_STUDY_2}/{directory_name}")
      ...
      for page_index, page_directory_name in enumerate(pages_list):
        ...
        alg_dir_name = f"{RESULTS_DIRECTORY_NAME_STUDY_2}/{directory_name}/{page_directory_name}"
        ...
        for alg_index, alg_file in enumerate(alg_files_list):
          if "alg-" in alg_file and ".json" in alg_file:
            ...
            #   continue
            image_name = alg_file.replace('.json', '-stable-diffusion-v6.png')
            image_full_path = f"{RESULTS_DIRECTORY_NAME_STUDY_2}/{directory_name}/{page_directory_name}/{image_name}"
            if SKIP_EXISTED_IMAGE_FILES and os.path.exists(image_full_path):
              print(f"File '{image_full_path}' already exists")
            else:
              alg_full_path = f"{RESULTS_DIRECTORY_NAME_STUDY_2}/{directory_name}/{page_directory_name}/{alg_file}"
