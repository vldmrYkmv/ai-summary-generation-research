    ...
    for index, directory_name in enumerate(files_list):
      ...
      for page_index, page_directory_name in enumerate(pages_list):
        ...
        for algorithm in algorithms_types:
          ...
          results_score[f'{directory_name}'][f'{page_directory_name}'][f'{algorithm}'] = [float(score[0]), float(score[1])]

    # print(results_score)
    save_text_to_file(json.dumps(results_score, ensure_ascii=False), f"../{RESULTS_DIRECTORY_NAME}/inception_score_i_a.json")
    end_time = time.time()
